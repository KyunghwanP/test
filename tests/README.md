...
네 가지가 있다 — `app`(지금까지) · `plain`(글꼴만 원본) · `orig`(Shadow DOM,
원본 + 스티키) · `frame`(사이트를 통째로 iframe).

`frame` 은 픽셀까지 같지만 **스티키를 못 넣는다** — 남의 문서라 CSS 를 넣을 수 없다.
구글이 삽입을 막아 두면 빈 화면이 뜨므로 안내와 원본보기를 위에 둔다.
